<template>
  <section>
    <div>
      <h3>Burnt <span class="explanation">(Job applications that didn't turn out so well...)</span></h3> 
      <!-- Carousel  -->
      <vue-horizontal-list :items="items">
        <template v-slot:default="{ item }">
          ...
          <div class="slide" :style="{ backgroundColor: randomColor() }">
            ...
            <!-- More info modal -->
            <JobAppCard id="card" v-bind:item="item" />
          </div>
        </template>
      </vue-horizontal-list>
    </div>
...
export default {
  ...
  // API call
  created() {
    ApplicationService.status(this.user.id, "Burnt").then(response => {
